...
class ConsolePage(QWidget):
    ...
    def setup_style(self):
        self.setStyleSheet("""
            ...
            QGroupBox {
                ...
                border-radius: 12px; margin-top: 6px; padding-top: 12px; font-weight: 500;
            }
            QGroupBox::title { subcontrol-origin: margin; left: 12px; padding: 0 6px; color: #cccccc; }
            QTreeWidget, QListWidget, QTextEdit, QTextBrowser { background: transparent; border: none; color: #cccccc; }
            QTreeWidget::item, QListWidget::item { padding: 3px 0; border-radius: 4px; }
            QTreeWidget::item:hover, QListWidget::item:hover { background: rgba(255, 255, 255, 0.05); }
            QTreeWidget::item:selected, QListWidget::item:selected { background: rgba(14, 99, 156, 0.3); }
            QPushButton {
                background: rgba(61, 61, 61, 0.5); border: 1px solid rgba(74, 74, 74, 0.5);
                border-radius: 16px; padding: 6px 12px; color: #cccccc;
            }
            QPushButton:hover { background: rgba(74, 74, 74, 0.7); }
            QTextEdit {
                background: rgba(30, 30, 30, 0.5); border: 1px solid rgba(255, 255, 255, 0.08);
                border-radius: 10px; padding: 10px;
            }
            QTextEdit:focus { border-color: #13a10e; }
    ...
    def setup_ui(self):
        ...
        self.status_label.setStyleSheet("""
            QLabel { color: #6a9955; font-size: 12px; padding: 2px 8px;
            background: rgba(37, 37, 38, 0.8); border-radius: 4px; }
        """)
        layout.addWidget(self.status_label)

        main_splitter = QSplitter(Qt.Horizontal)
        main_splitter.setHandleWidth(1)
        main_splitter.setStyleSheet("QSplitter::handle { background: rgba(255, 255, 255, 0.05); margin: 4px 0; }")

        main_splitter.addWidget(self._create_left_panel())
        main_splitter.addWidget(self._create_code_panel())
        main_splitter.addWidget(self._create_chat_panel())
    ...
    def _create_code_panel(self):
        ...
        tab_bar.setStyleSheet("QWidget { background: rgba(37, 37, 38, 0.8); border-radius: 8px; }")
        ...
        self.edit_mode_btn.setStyleSheet("QPushButton { background: transparent; border: none; color: #a0a0a0; padding: 4px 12px; font-size: 12px; } QPushButton:hover { color: #ffffff; } QPushButton:checked { color: #13a10e; }")
        ...
        self.submit_btn.setStyleSheet("QPushButton { background: #13a10e; border: none; border-radius: 12px; padding: 4px 12px; color: white; font-size: 12px; } QPushButton:hover { background: #16c60c; } QPushButton:disabled { background: #4a4a4a; }")
    ...
    def _create_chat_panel(self):
        ...
        self.message_input.setStyleSheet("QLineEdit { background: rgba(45, 45, 45, 0.6); border: none; border-radius: 20px; padding: 0 16px; color: #e0e0e0; font-size: 13px; min-height: 36px; max-height: 36px; }")
        ...
        self.send_btn.setStyleSheet("QPushButton { background: #0e639c; border: none; border-radius: 18px; color: white; font-weight: 500; } QPushButton:hover { background: #1177bb; }")
    ...
    def _create_tab_button(self, text, checked=False):
        ...
        btn.setStyleSheet("QPushButton { background: transparent; border: none; border-bottom: 2px solid transparent; padding: 4px 12px; color: #a0a0a0; font-size: 11px; } QPushButton:hover { color: #e0e0e0; } QPushButton:checked { color: #13a10e; border-bottom-color: #13a10e; }")
        return btn
    ...
    def toggle_edit_mode(self):
        ...
        if self.edit_mode:
            if self.current_editing_file:
                ...
                self.code_display.setStyleSheet("QTextEdit { background: #1e1e1e; border: 2px solid #13a10e; border-radius: 8px; color: #d4d4d4; padding: 16px; }")
                self.submit_btn.setVisible(True)
                self.add_log("✏️ 编辑模式已开启", "info")
            else:
                self.edit_mode_btn.setChecked(False)
                self.add_log("请先双击打开一个文件", "warning")
        else:
            self.code_display.setReadOnly(True)
            self.code_display.setStyleSheet("QTextEdit { background: #1e1e1e; border: none; color: #d4d4d4; padding: 16px; }")
            self.submit_btn.setVisible(False)
            self.submit_btn.setEnabled(False)

    def on_content_changed(self):
        if self.edit_mode and self.current_editing_file:
            current = self.code_display.toPlainText()
            self.submit_btn.setEnabled(current != self.original_content)
    ...
    def submit_manual_changes(self):
        ...
        if not syntax_ok:
            ...
            QMessageBox.warning(self, "语法错误", f"代码存在语法错误：\n\n{syntax_error}")
            return

        self.add_log("✅ 语法检查通过", "success")
        self.show_diff(rel_path, self.original_content, new_content)

        reply = QMessageBox.question(self, "提交修改", f"即将保存对 {rel_path} 的修改。\n\n是否让 AI 分析影响并自动更新相关文件？", QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel)
        ...
        if reply == QMessageBox.Yes and self.worker:
            self.add_log("🔍 正在分析影响...", "info")
            self.worker.analyze_impact_and_update(file_path=rel_path, old_content=self.original_content, new_content=new_content)
    ...
    def set_project_path(self, path):
        self.project_path = Path(path) if isinstance(path, str) else path
    ...
    def refresh_file_tree(self):
        ...
        if not self.project_path or not self.project_path.exists():
            return
        root = QTreeWidgetItem([self.project_path.name])
        self.file_tree.addTopLevelItem(root)
        self._add_directory_items(root, self.project_path)
        root.setExpanded(True)

    def _add_directory_items(self, parent, path: Path):
        try:
            for item in sorted(path.iterdir()):
                ...
                if item.is_dir():
                    ...
                    parent.addChild(QTreeWidgetItem([item.name]))
    ...
    def on_file_double_clicked(self, item, column):
        ...
        if file_path.is_file():
            try:
                ...
                self.current_editing_file = {"full_path": str(file_path), "rel_path": str(file_path.relative_to(self.project_path)), "name": file_path.name}
    ...
    def show_diff(self, file_path, old_content, new_content):
        import difflib
        diff = '\n'.join(difflib.unified_diff(old_content.splitlines(), new_content.splitlines(), fromfile=f'原: {file_path}', tofile=f'新: {file_path}', lineterm=''))
        self.diff_display.setPlainText(diff)
        self.switch_tab("diff")
        self.add_log(f"显示 {file_path} 的差异", "info")
    ...
    def on_state_changed(self, state: str):
        state_map = {"idle": "● 就绪", "planning": "⏳ 规划中", "waiting_confirm": "⏸ 等待确认", "executing": "🔄 执行中", "tool_executing": "🔧 工具执行中", "modifying": "✏️ 修改中", "paused": "⏸ 已暂停", "cancelled": "✖ 已取消"}
    ...
    def update_plan(self, steps):
        ...
        for step in steps:
            self.step_list.addItem(QListWidgetItem(f"⏳ {step}"))
    ...
    def update_step_status(self, step_id, status, message=""):
        ...
        if 0 <= index < self.step_list.count():
            ...
            text = f"{icon} {message}" if message else f"{icon} 步骤{step_id}"
            self.step_list.item(index).setText(text)
    ...
    def show_options(self, options, context):
        ...
        if options:
            for opt in options:
                ...
                btn.setStyleSheet("QPushButton { background: #3d3d3d; padding: 6px 12px; border-radius: 12px; } QPushButton:hover { background: #4d4d4d; }")
    ...
    def _on_option_clicked(self, option):
        ...
        if self.worker:
            self.worker.handle_user_input(option)
