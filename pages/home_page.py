...
注意：
- 不要预设选项，根据任务内容灵活提问
- 如果任务是游戏，问引擎/框架；如果是数据分析，问数据格式和工具；如果是 Web，问前后端框架
- 问题要简洁友好
...
"""
        ...
        try:
            question = self.llm_client.chat([
                {"role": "system", "content": "你是项目初始化助手，通过对话帮助用户确定项目约定。问题要简洁，一次只问1-2个关键点。"},
...
"""
            ...
            try:
                response = self.llm_client.chat([
                    {"role": "system", "content": "你是项目初始化助手，输出必须是合法的 JSON 格式。"},
...
"""
            ...
            try:
                self.agents_md = self.llm_client.chat([
                    {"role": "system", "content": "你是项目架构专家，生成项目约定文档。"},
                    {"role": "user", "content": generate_prompt}
                ])
            except Exception as e:
                self.agents_md = f"""# 项目约定
...
class HomePage(QWidget):
    ...
    def setup_ui(self):
        ...
        self.new_btn.setStyleSheet("""
            ...
            QPushButton:hover { background: #1a75e8; }
        """)
...
class ProjectItemWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        date.setStyleSheet("color: #888888; font-size: 12px;")
        layout.addWidget(date)
        ...
        if total > 0:
            progress = QLabel(f"{completed}/{total} 步骤")
            progress.setStyleSheet("color: #888888; font-size: 12px;")
            layout.addWidget(progress)
